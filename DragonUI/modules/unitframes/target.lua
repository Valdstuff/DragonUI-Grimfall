...
-- Target threat/aggro glow (TargetFrameFlash). Defaults ON when unset.
local function IsThreatGlowEnabled()
    local cfg = addon.db and addon.db.profile and addon.db.profile.unitframe and addon.db.profile.unitframe.target
    if cfg and cfg.enableThreatGlow ~= nil then
        return cfg.enableThreatGlow
    end
    return true
end

-- Apply the current setting to TargetFrameFlash immediately.
local function RefreshTargetThreatGlow()
    local flash = _G.TargetFrameFlash
    if not flash then return end
    if IsThreatGlowEnabled() then
        flash:SetAlpha(0.7)
    else
        flash:SetAlpha(0)
        flash:Hide()
...
addon.RefreshTargetThreatGlow = RefreshTargetThreatGlow
        ...
        if not ctx.Module.threatHooked then
            hooksecurefunc("TargetFrame_CheckClassification",
                function(self, forceNormalTexture)
                    ...
                    if threatFlash then
                        if not IsThreatGlowEnabled() then
            ...
            -- Keep the flash hidden when the glow is disabled, even when the
            -- threat system tries to show/pulse it.
            if _G.TargetFrameFlash then
                hooksecurefunc(_G.TargetFrameFlash, "Show", function(self)
                    if not IsThreatGlowEnabled() then
                        self:SetAlpha(0)
                        self:Hide()
                    end
